...
- [ ] Read `packages/core/src/parser/markdown-parser.ts` and list the states the AsciiDoc parser must mirror: comments, open block, pending ignore, pending diagram, bare mermaid, unclosed block.
- [ ] Read `packages/core/src/ast.ts` and confirm `BlockNode.inArc42Fence`, `DiagramNode`, `IgnoreNode`, and `BareMermaidNode` need no new fields.
- [ ] Read `packages/core/src/arc42.ts` `parseArchitectureDocument` and `packages/workspace-fs/src/index.ts` `discoverFiles`.
- [ ] Read `packages/workspace-fs/src/git-diff.ts` and note both `.arc42.md` filters.
- [ ] Read `packages/core/src/path-utils.ts` `chapterNumberFromFile` and the W015 test that skips files not ending in `.arc42.md`.
- [ ] Read `packages/cli/src/renderer/markdown.ts` and `packages/cli/src/renderer/index.ts` for the get-renderer pattern.
- [ ] Read `packages/cli/src/cli.ts` command dispatch, `runGet`, and the serve `watch` callback.
- [ ] Read `packages/cli/src/chapters.ts` `CHAPTERS` and `filename`, plus `packages/cli/src/guide.ts`.
- [ ] Read `packages/core/src/explain.ts` ignore syntax and `packages/core/src/validator/rules/w016-block-not-in-arc42-fence.ts` and `w017-bare-mermaid-block.ts`.
- [ ] Read `packages/web/src/utils.ts` `basename` and `packages/skill/SKILL.md`.

### Entrance criteria

- [ ] The syntax table above matches the Markdown parser's nodes.
- [ ] Every command in the coverage table has a file to change.
- [ ] Test files named below do not already exist.

## Plan

### Tasks

- [ ] Lock the grammar in the Syntax section. Do not add a second `[arc42]` wrapper around typed blocks.
- [ ] Lock file touch list:
...
- [ ] Sequence the Code waves below. Each wave is red, then green, then `pnpm test` for the packages that wave touches.

### Entrance criteria

- [ ] Grammar, file list, and wave order are unchanged from this section.
- [ ] Wave 1 is the only wave that may start.
...
- [ ] **Red.** Create `packages/core/tests/parser-asciidoc.test.ts`. Copy the helpers in `packages/core/tests/parser.test.ts` (`blocks`, `headings`, `prose`, `ignores`) and call `parseAsciiDoc`. Cover at least 25 cases:
...
- [ ] **Red.** Create `packages/core/tests/parser-equivalence.test.ts` with at least 15 pairs. Each pair parses Markdown with `parseMarkdown` and the translated AsciiDoc with `parseAsciiDoc`, then compares kind, block type, attributes, heading level and text, diagram id/view/notation/source, and ignore rule/reason:
...
- [ ] **Green.** Add `packages/core/src/parser/asciidoc-parser.ts`.
...
- [ ] **Green.** In `parseArchitectureDocument`, if `filePath` ends with `.arc42.adoc`, call `AsciiDocParser`. Otherwise keep `MarkdownParser`.
- [ ] **Verify.** `pnpm --filter @arc42/core test` passes, including the new files. Existing `parser.test.ts` is unchanged.

### Wave 2 — Load AsciiDoc files

- [ ] **Red.** In `packages/workspace-fs/tests/workspace-fs.test.ts`, assert `discoverFiles` returns `.arc42.md` and `.arc42.adoc` and skips `notes.adoc`. Assert `readWorkspaceDocuments` returns both architecture files.
- [ ] **Red.** In `packages/core/tests/validator-w015.test.ts`, a `06-runtime-view.arc42.adoc` whose first heading is not the chapter title emits W015. `06-runtime-view.md` still does not.
- [ ] **Red.** In `packages/core/tests/validator-e016.test.ts`, an interface documented in `05-building-blocks.arc42.adoc` is accepted, and an actor in that same file emits E016.
- [ ] **Green.** `discoverFiles` in `packages/workspace-fs/src/index.ts` keeps names ending in `.arc42.md` or `.arc42.adoc`.
- [ ] **Green.** `chapterNumberFromFile` in `packages/core/src/path-utils.ts` accepts both suffixes, then the existing `^(\d{2})-` rule.
- [ ] **Verify.** `pnpm --filter @arc42/core test` and `pnpm --filter @arc42/workspace-fs test` pass.

### Wave 3 — Validate on the CLI

- [ ] **Red.** Add a CLI test that writes one valid `.arc42.adoc` chapter and expects `arc42 validate` exit 0, then a duplicate-id error and exit 1, with `file` ending in `.arc42.adoc`.
- [ ] **Red.** A directory with one `.arc42.md` error and one `.arc42.adoc` error reports both paths.
- [ ] **Red.** Extend `packages/core/tests/validator-w016.test.ts`: a `[arc42.building-block]` block does not emit W016.
- [ ] **Red.** A bare `[source,mermaid]` block emits W017.
- [ ] **Red.** An `[arc42.ignore]` for a hint suppresses that hint the same way `:::ignore` does.
- [ ] **Green.** Update the `validate` string in `packages/cli/src/help.ts` to say `*.arc42.md` and `*.arc42.adoc`.
- [ ] **Green.** Adjust W016 and W017 `docs.description` so each sentence that tells the author what to write also names the AsciiDoc form: `[arc42.<type>]` / `----` for W016, `[arc42.diagram]` plus `[source,…]` for W017. Keep the Markdown sentence.
- [ ] **Verify.** `pnpm --filter @arc42/cli test` and the core validator tests pass. `arc42 validate --dir examples/bookstore-backend` still exits 0.

### Wave 4 — `arc42 get --format asciidoc`

- [ ] **Red.** Create `packages/cli/tests/renderer-asciidoc.test.ts` with at least 29 tests. Follow `packages/cli/tests/renderer.test.ts` for how a `GetResult` is built.
...
- [ ] **Green.** Add `packages/cli/src/renderer/asciidoc.ts` class `AsciiDocGetRenderer implements GetRenderer`.
...
- [ ] **Green.** Register the instance in `packages/cli/src/renderer/index.ts` `builtinGetRenderers`.
- [ ] **Green.** In `commandHelp("get")`, list `asciidoc` next to `text`, `json`, and `markdown`. Add one `arc42 get --format asciidoc` example to `packages/cli/README.md`.
- [ ] **Verify.** CLI renderer tests pass. `arc42 get --format markdown` output is unchanged.

### Wave 5 — Template transform, init, round-trip

- [ ] **Red.** Create `packages/core/tests/renderer-asciidoc-roundtrip.test.ts` with at least 5 read → AsciiDoc source → read cases and 3 preservation cases:
...
- [ ] **Green.** Add `renderAsciiDocSource(documents: DocumentAst[]): string` in `packages/core/src/parser/asciidoc-writer.ts` (or `packages/cli/src/template-asciidoc.ts` if it only serves the CLI). It emits the Syntax section from an AST: `=` headings, prose lines, `[arc42.<type>]` blocks, `[arc42.ignore]`, `[arc42.diagram]` plus `[source,…]`. This is what the round-trip tests call. It is not `AsciiDocGetRenderer`.
- [ ] **Green.** Add `markdownTemplateToAsciiDoc(template: string): string` used by init and guide:
  - `#` / `##` / `###` at line start → `=` / `==` / `===`
  - HTML comments → `////` block comments
  - ` ```arc42 ` … `:::type` … `:::` … ` ``` ` → `[arc42.type]` … `----`
  - `:::ignore` and `:::diagram` → the Syntax forms
  - `](….arc42.md)` → `](….arc42.adoc)`
- [ ] **Green.** Add `runInit` in `packages/cli/src/cli.ts`, dispatched before workspace discovery the same way `guide` is, so init does not warn about a missing workspace.
...
- [ ] **Green.** Add `init` to `COMMANDS` and `commandHelp("init")` in `packages/cli/src/help.ts`.
- [ ] **Verify.** Round-trip tests and init tests pass. `arc42 init --format asciidoc` on a temp dir validates.

### Wave 6 — Explain, guide, skill

- [ ] **Red.** Help or CLI tests:
...
- [ ] **Green.** In `packages/core/src/explain.ts`, append an AsciiDoc example to each block explanation, each diagram explanation, and `IGNORE_DATA.syntax`. Build the block example from the type name: `[arc42.<type>]`, `----`, one line per required field, `----`.
- [ ] **Green.** `runGuide` accepts `--format markdown|asciidoc` on `chapter`. Pass the chapter template through `markdownTemplateToAsciiDoc` when the format is asciidoc.
- [ ] **Green.** In `packages/cli/src/guide.ts`, describe chapter files as `*.arc42.md` or `*.arc42.adoc` and say the workspace uses one suffix.
- [ ] **Green.** In `packages/skill/SKILL.md`, name both suffixes in the description and workflow, and show the `[arc42.ignore]` example next to the existing `:::ignore` fence.
- [ ] **Verify.** Explain, rules, and guide tests pass. `arc42 guide chapter 1` Markdown output is unchanged apart from the suffix wording in the migration guide.

### Wave 7 — Diff, coverage, serve, build

- [ ] **Red.** In `packages/workspace-fs/tests/git-diff.test.ts`, a repo whose only change is `architecture.arc42.adoc` appears in `currentDocuments` for a working-tree diff, a staged diff, and a diff against a parent revision.
- [ ] **Red.** A CLI coverage test: an AsciiDoc building block with `path: src/api` makes `arc42 coverage` report `src/api` as claimed.
- [ ] **Red.** `basename("05-building-blocks.arc42.adoc")` in `packages/web/src/utils.ts` returns `05-building-blocks`.
- [ ] **Red.** Serve help mentions `.arc42.adoc`. A built site payload includes a document whose `filePath` ends with `.arc42.adoc`.
- [ ] **Green.** Replace both `.arc42.md` filters in `packages/workspace-fs/src/git-diff.ts` with the same suffix check as `discoverFiles`.
- [ ] **Green.** In `runServe`, reload when the changed name ends with `.arc42.md` or `.arc42.adoc`.
- [ ] **Green.** `basename` strips `.arc42.adoc` before the `.md` fallback.
- [ ] **Green.** Help strings for `validate`, `serve`, and `diff` name both suffixes where they currently say only `*.arc42.md`.
- [ ] **Verify.** workspace-fs, web, and CLI tests pass. `arc42 build --out <tmp>` on an AsciiDoc init directory writes a site whose workspace JSON lists the twelve `.arc42.adoc` files.

## Commit

- [ ] Review the diff against the file list in Plan. No drive-by edits.
- [ ] `pnpm run check` passes.
- [ ] `pnpm test` passes.
- [ ] `arc42 validate --dir examples/bookstore-backend` exits 0.
- [ ] Commit subject: `feat: support AsciiDoc architecture documents`
- [ ] Commit body lists parser, discovery, validate, get, init, explain, guide, diff, coverage, serve, and build.

### Entrance criteria

- [ ] Every Code wave is checked.
- [ ] Success criteria below are checked.

## Success criteria

### Read

- [ ] Both parsers produce the same blocks, headings, diagrams, and ignores for equivalent content
- [ ] The CLI selects the parser from the file extension
- [ ] Mixed workspaces (`.arc42.md` and `.arc42.adoc`) load together
- [ ] **25+ AsciiDoc parser unit tests** in `parser-asciidoc.test.ts`
- [ ] **15+ cross-parser equivalence tests** in `parser-equivalence.test.ts`

### Validate

- [ ] `arc42 validate` reports diagnostics for `.arc42.adoc` with path and line
- [ ] `arc42 validate`, `arc42 get`, and `arc42 explain` still behave the same on Markdown workspaces
- [ ] Chapter assignment (E016) and chapter titles (W015) work for `.arc42.adoc` filenames
- [ ] W016 does not fire for `[arc42.<type>]`. W017 fires for a bare `[source,…]`

### Write

- [ ] `arc42 get --format asciidoc` produces AsciiDoc for the workspace and for one element
- [ ] Every element kind is rendered
- [ ] Cross-references are preserved, and a missing reference does not throw
- [ ] **29+ renderer unit tests** in `renderer-asciidoc.test.ts`
- [ ] **5+ round-trip tests** in `renderer-asciidoc-roundtrip.test.ts`
- [ ] Help text and the CLI README show the `asciidoc` format

### Init

- [ ] `arc42 init --format asciidoc` writes twelve chapter files
- [ ] That directory validates with exit 0
- [ ] Existing files are not overwritten

### Every other command

- [ ] `arc42 explain` shows Markdown and AsciiDoc syntax for blocks, diagrams, and ignore
- [ ] `arc42 guide chapter <n> --format asciidoc` prints an AsciiDoc starter
- [ ] `arc42 rules` describes the AsciiDoc form of W016 and W017
- [ ] `arc42 diff` includes `.arc42.adoc` changes
- [ ] `arc42 coverage` claims paths declared in AsciiDoc elements
- [ ] `arc42 serve` reloads on `.arc42.adoc` edits
- [ ] `arc42 build` publishes an AsciiDoc workspace
- [ ] The skill workflow documents both suffixes

### Regressions

- [ ] Existing tests pass, except assertions that the only architecture suffix is `.arc42.md`
- [ ] `arc42 validate --dir examples/bookstore-backend` still succeeds
